...
Gates are aggregate: the sarcastic side must be strong, the sincere side is
allowed a couple of false positives but must stay low on average. The shipped
model is trained with the three sarcasm_*_augment.jsonl files (sincere
positive-surface negatives, polite-surface sarcastic hard positives, and
minimal-pair sincere negatives that reuse the same polite openers) — drop any
of them and one side of this battery regresses.
"""
import os
...
def test_sincere_mostly_below_threshold(clf, scores):
    # Was xfail until 2026-07-07: the tweet_eval-trained model over-fired on
    # sincere positive-surface texts ('Thank you so much…' scored ~0.94). Fixed
    # by the sincere + minimal-pair augments; a regression here means the
    # augment files were dropped from training.
    _, sinc = scores
