...
const CDM_STATES = [
  ...
  { id:  2, name: 'PRAISE',         short: 'PRAISE',    color: '34,197,94'   },
  { id:  3, name: 'HELP_REQUEST',   short: 'HELP',      color: '59,130,246'  },
  { id:  4, name: 'HUMOR',          short: 'HUMOR',     color: '251,191,36'  },
  { id:  5, name: 'TENSION',        short: 'TENSION',   color: '249,115,22'  },
  { id:  6, name: 'CONFLICT',       short: 'CONFLICT',  color: '239,68,68'   },
  { id:  7, name: 'ARGUMENT',       short: 'ARGUMENT',  color: '220,38,38'   },
  { id:  8, name: 'WITHDRAWAL',     short: 'WITHDRAW',  color: '168,85,247'  },
  { id:  9, name: 'RECONCILIATION', short: 'RECONCILE', color: '139,92,246'  },
  { id: 10, name: 'CURIOSITY',      short: 'CURIOSITY', color: '14,165,233'  },
  { id: 11, name: 'ASSERTIVENESS',  short: 'ASSERT',    color: '99,102,241'  },
  { id: 12, name: 'EMPATHY',        short: 'EMPATHY',   color: '16,185,129'  },
  { id: 13, name: 'FRUSTRATION',    short: 'FRUSTRAT',  color: '234,88,12'   },
  ...
  return (
    <div style={{ marginBottom: 7 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
        <span style={{
          fontSize: '0.74rem', color: highlight ? `rgb(${color})` : 'rgba(var(--ig-ink-rgb),0.7)',
    ...
    <div style={S.stageCard}>
      ...
      {isEmpty ? (
        ...
        <div style={{ marginTop: 12 }}>
          ...
          {entries.length > 8 && (
            <div style={{ fontSize: '0.68rem', color: 'rgba(var(--ig-ink-rgb),0.3)', marginTop: 6 }}>
  ...
  return (
    <div style={S.stageCard}>
      ...
      <div style={{ marginTop: 14 }}>
        ...
        <div style={{ marginTop: 14, padding: '10px 14px', background: 'rgba(var(--ig-ink-rgb),0.04)', borderRadius: 10 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '0.75rem', color: 'rgba(var(--ig-ink-rgb),0.5)' }}>Compound Score</span>
            <span style={{ fontSize: '1.1rem', fontWeight: 800, color: `rgb(${sentColor})` }}>
              {compound.toFixed(4)}
            </span>
          </div>
          <div style={{ marginTop: 8, height: 6, borderRadius: 3, background: 'rgba(var(--ig-ink-rgb),0.07)', position: 'relative' }}>
            <div style={{
            ...
            <div style={{ position: 'absolute', left: '50%', top: -2, width: 2, height: 10, background: 'rgba(var(--ig-ink-rgb),0.3)' }} />
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
            <span style={{ fontSize: '0.6rem', color: 'rgba(var(--ig-ink-rgb),0.3)' }}>-1.0</span>
            <span style={{ fontSize: '0.6rem', color: 'rgba(var(--ig-ink-rgb),0.3)' }}>0</span>
            <span style={{ fontSize: '0.6rem', color: 'rgba(var(--ig-ink-rgb),0.3)' }}>+1.0</span>
          </div>
        </div>
      </div>
        ...
        {entries.map(([model, val]) => {
          ...
          return (
            <div key={model} style={{ marginBottom: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                <span style={{ fontSize: '0.75rem', color: 'rgba(var(--ig-ink-rgb),0.7)' }}>{model}</span>
                <span style={{
...
function ValenceBar({ value }) {
  ...
  return (
    <div style={{ position: 'relative', height: 6, borderRadius: 3, background: 'rgba(var(--ig-ink-rgb),0.07)', margin: '4px 0 2px' }}>
      <div style={{ position: 'absolute', left: '50%', top: 0, width: 1, height: '100%', background: 'rgba(var(--ig-ink-rgb),0.2)' }} />
      <div style={{
...
function ContextCard({ decision, context }) {
  ...
  const row = (label, val, color = 'rgba(var(--ig-ink-rgb),0.55)') => (
  ...
  return (
    <div style={{ ...S.stageCard, borderColor: 'rgba(255,100,150,0.3)', background: 'rgba(255,100,150,0.04)' }}>
      <div style={S.stageHeader}>
        ...
        <div>
          ...
          <div style={S.stageDesc}>
            Looks beyond this one message — tracks the conversation's mood, its social state
            (CDM), and similar past moments (episodic memory), and feeds 40 extra features into
      ...
      <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {hasSnapshot && (
          <div style={{ padding: '10px 12px', background: 'rgba(var(--ig-ink-rgb),0.04)', borderRadius: 8 }}>
            <div style={{ fontSize: '0.68rem', color: 'rgba(var(--ig-ink-rgb),0.35)', marginBottom: 2, textTransform: 'uppercase', letterSpacing: '0.07em' }}>
              Conversation State
            </div>
            <div style={S.ctxHint}>
              What the conversation felt like right before this message — carried forward so one
              message is never judged in isolation.
            </div>
            {row('Previous emotion', cs.prev_emotion || 'neutral',
              `rgb(${rgb(cs.prev_emotion || 'neutral')})`)}
            <div style={S.contextRow} title="Exponential moving average of recent message sentiment — the running mood of the conversation">
              <span style={S.ctxLabel}>Running mood (EMA valence)</span>
            ...
            {cs.ce_available && (
              <>
                <div style={{ height: 1, background: 'rgba(var(--ig-ink-rgb),0.07)', margin: '8px 0' }} />
                <div style={{ fontSize: '0.68rem', color: 'rgba(var(--ig-ink-rgb),0.35)', marginBottom: 2, textTransform: 'uppercase', letterSpacing: '0.07em' }}>
                  Episodic Memory (Qdrant)
                </div>
                <div style={S.ctxHint}>
                  Vector search over this user's past messages — "the last time this topic came
                  up, how did it feel?"
                </div>
                <div style={S.contextRow} title="Net sentiment of the retrieved similar past moments (positive share minus negative share)">
                  <span style={S.ctxLabel}>Past mood on this topic</span>
                  <div style={{ flex: 1, margin: '0 12px' }}><ValenceBar value={histValence} /></div>
                  <span style={{ ...S.ctxValue, color: histValence >= 0 ? 'rgb(0,210,120)' : 'rgb(255,82,82)' }}>
                    {histValence >= 0 ? '+' : ''}{(histValence * 100).toFixed(1)}%
                  </span>
                </div>
                {row('Topic resonance',
                  cs.topic_resonance > 0
                    ? `${(cs.topic_resonance * 100).toFixed(0)}% match with past conversations`
                    : 'New topic — nothing similar in memory',
                  cs.topic_resonance > 0.3 ? 'rgb(255,200,80)' : 'rgba(var(--ig-ink-rgb),0.4)')}
                {row('Mood volatility',
                  cs.volatility > 0.01
                    ? `${(cs.volatility * 100).toFixed(1)}% — ${cs.volatility > 0.1 ? 'swinging hard' : cs.volatility > 0.03 ? 'shifting' : 'mostly steady'}`
                    : 'Stable',
                  cs.volatility > 0.1 ? 'rgb(255,150,50)' : 'rgba(var(--ig-ink-rgb),0.45)')}
              </>
            )}
            ...
            {cs.cdm_available && cs.cdm_state_probs && (
              <>
                <div style={{ height: 1, background: 'rgba(var(--ig-ink-rgb),0.07)', margin: '8px 0' }} />
                <div style={{ fontSize: '0.68rem', color: 'rgba(var(--ig-ink-rgb),0.35)', marginBottom: 2, textTransform: 'uppercase', letterSpacing: '0.07em' }}>
                  CDM — Conversation Dynamics
                </div>
                <div style={S.ctxHint}>
                  A hidden Markov model classifies the social state of the exchange across 15
                  states (warmth, tension, conflict, humor…). ▶ marks where the conversation is
                  right now; the bars are the model's belief in each state.
                </div>
                {CDM_STATES.map(({ id, short, color }) => {
                  const prob = cs.cdm_state_probs[id] ?? 0;
                  const isCurrent = curStateIdx === id;
                  return (
                    <div key={id} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      <span style={{
                        ...
                        color: isCurrent ? `rgb(${color})` : 'rgba(var(--ig-ink-rgb),0.35)',
                        flexShrink: 0,
                      }}>
                        {isCurrent ? '▶ ' : ''}{short}
                      </span>
                      <div style={{ flex: 1, height: 4, borderRadius: 2, background: 'rgba(var(--ig-ink-rgb),0.07)' }}>
                        <div style={{
                      ...
                      <span style={{ fontSize: '0.62rem', color: isCurrent ? `rgb(${color})` : 'rgba(var(--ig-ink-rgb),0.3)', width: 30, textAlign: 'right' }}>
                        {(prob * 100).toFixed(0)}%
                      </span>
                    </div>
                  );
                ...
                {cs.cdm_residency > 0 && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}
                    title="How long the conversation has stayed in the current state — high residency means an entrenched dynamic">
                    <span style={{ width: 58, fontSize: '0.62rem', color: 'rgba(var(--ig-ink-rgb),0.35)', flexShrink: 0 }}>residency</span>
                    <div style={{ flex: 1, height: 3, borderRadius: 2, background: 'rgba(var(--ig-ink-rgb),0.07)' }}>
                      <div style={{
                        height: '100%', borderRadius: 2,
                        width: `${Math.min(cs.cdm_residency * 100, 100)}%`,
                        background: `rgba(${CDM_STATES[curStateIdx]?.color ?? '150,150,150'},0.6)`,
                      }} />
                    </div>
                    <span style={{ fontSize: '0.62rem', color: 'rgba(var(--ig-ink-rgb),0.3)', width: 30, textAlign: 'right' }}>
            ...
            {!cs.ce_available && (
              <div style={{ fontSize: '0.72rem', color: 'rgba(var(--ig-ink-rgb),0.3)', fontStyle: 'italic', marginTop: 4 }}>
                Episodic memory didn't respond within the aggregation window for this message —
                the prediction used only the running conversation mood above.
              </div>
            )}
          </div>
        )}
        ...
        {contextPct != null && (
          <div style={S.contextRow}
            title="Share of the final prediction driven by conversation context rather than the raw text models — the meta-learner decides this per message">
            <span style={S.ctxLabel}>Context influence on verdict</span>
            <div style={{ flex: 1, height: 6, borderRadius: 3, background: 'rgba(var(--ig-ink-rgb),0.07)', margin: '0 12px' }}>
              <div style={{
        ...
        {hasShift && (
          <div style={S.contextChip}>
            ...
            <span style={{ color: 'rgba(var(--ig-ink-rgb),0.4)', margin: '0 4px' }}>→</span>
            <strong style={{ color: `rgb(${rgb(contextShift.to)})` }}>{contextShift.to}</strong>
            <span style={{ marginLeft: 6, fontSize: '0.68rem', padding: '1px 7px', background: 'rgba(var(--ig-ink-rgb),0.08)', borderRadius: 10 }}>
        ...
        {hasSarcasm && (
          <div style={{ ...S.contextChip, background: 'rgba(255,200,0,0.08)', borderColor: 'rgba(255,200,0,0.2)' }}>
            <span style={{ fontSize: '0.8rem' }}>😏</span>
            <span style={{ color: 'rgba(var(--ig-ink-rgb),0.7)' }}>Sarcasm / irony — </span>
        ...
        {hasConflict && (
          <div style={{ ...S.contextChip, background: 'rgba(255,82,82,0.08)', borderColor: 'rgba(255,82,82,0.2)' }}>
            ...
            <span style={{ color: 'rgba(var(--ig-ink-rgb),0.7)' }}>Emotional conflict: </span>
        ...
        {hasReason && (
          <div style={{ padding: '10px 12px', background: 'rgba(var(--ig-ink-rgb),0.04)', borderRadius: 8 }}>
            <div style={{ fontSize: '0.68rem', color: 'rgba(var(--ig-ink-rgb),0.4)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.07em' }}>
              LLM Reasoning
            </div>
            {reasoning.type && (
              <div style={{ fontSize: '0.78rem', color: 'rgba(var(--ig-ink-rgb),0.8)', marginBottom: 4 }}>
                <strong style={{ color: 'rgb(255,100,150)' }}>{reasoning.type}</strong>
                {reasoning.details && <> — {reasoning.details}</>}
              </div>
            )}
            {reasoning.action && (
              <div style={{ fontSize: '0.72rem', color: 'rgba(var(--ig-ink-rgb),0.45)', fontStyle: 'italic' }}>
                {reasoning.action}
              </div>
            )}
          </div>
...
function DecisionTrace({ trace, dominant }) {
  ...
  return (
    <div style={{ marginTop: 14 }}>
      <div style={S.miniLabel}>Decision path</div>
      <div style={{ fontSize: '0.68rem', color: 'rgba(var(--ig-ink-rgb),0.40)', lineHeight: 1.5, marginBottom: 8 }}>
        Every mechanism that touched (or declined to touch) this verdict, in order.
      </div>
      {trace.map((t, i) => {
        ...
        return (
          <div key={i} style={{ display: 'flex', gap: 10 }}>
            ...
            <div style={{ paddingBottom: isLast ? 0 : 10, minWidth: 0 }}>
              <div style={{ fontSize: '0.76rem', fontWeight: 700, color: 'rgba(var(--ig-ink-rgb),0.80)' }}>
                {t.step}
                {m.label && (
                  <span style={{ marginLeft: 6, fontSize: '0.62rem', fontWeight: 600, color: `rgb(${m.color})`,
                                 background: `rgba(${m.color},0.10)`, border: `1px solid rgba(${m.color},0.25)`,
                                 borderRadius: 5, padding: '1px 6px', verticalAlign: '1px' }}>
                    {m.label}
                  </span>
                )}
                {t.label && (
                  <span style={{ marginLeft: 8, fontSize: '0.72rem', fontWeight: 700, color: `rgb(${rgb(t.label)})`, textTransform: 'capitalize' }}>
                    {t.label}{t.confidence != null ? ` ${(t.confidence * 100).toFixed(0)}%` : ''}
                  </span>
                )}
              </div>
              <div style={{ fontSize: '0.70rem', color: 'rgba(var(--ig-ink-rgb),0.48)', lineHeight: 1.45, marginTop: 1 }}>
                {t.detail}
              </div>
            </div>
          </div>
      ...
      <div style={{ display: 'flex', gap: 16, marginTop: 16, alignItems: 'center' }}>
        <div style={{ position: 'relative', width: 80, height: 80, flexShrink: 0 }}>
          <svg width="80" height="80" style={{ transform: 'rotate(-90deg)' }}>
            <circle cx="40" cy="40" r="32" fill="none" stroke="rgba(var(--ig-ink-rgb),0.07)" strokeWidth="7" />
            <circle
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: '0.65rem', color: 'rgba(var(--ig-ink-rgb),0.4)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 6 }}>
            Dominant Emotion
          </div>
  ...
  return (
    <div style={S.stageCard}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 0, marginTop: 16, overflowX: 'auto', paddingBottom: 4 }}>
        {all.map((item, i) => {
          ...
          return (
            <React.Fragment key={i}>
              {i > 0 && (
                <div style={{ width: 20, height: 2, flexShrink: 0, background: 'rgba(var(--ig-ink-rgb),0.15)' }} />
              )}
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                ...
                <span style={{
                  ...
                  color: isCurrent ? `rgb(${emoRgb})` : 'rgba(var(--ig-ink-rgb),0.45)',
        ...
        {hasLSTM && predicted && (
          <>
            <div style={{ width: 24, height: 2, flexShrink: 0, background: `rgba(${predRgb},0.4)`, borderTop: '2px dashed rgba(var(--ig-ink-rgb),0.2)' }} />
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, flexShrink: 0 }}>
              <div style={{
      ...
      {hasLSTM && top5Entries.length > 0 && (
        <div style={{ marginTop: 18, paddingTop: 14, borderTop: '1px solid rgba(var(--ig-ink-rgb),0.07)' }}>
          <div style={{ fontSize: '0.72rem', color: 'rgba(var(--ig-ink-rgb),0.4)', marginBottom: 10, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
      ...
      {!hasLSTM && (
        <div style={{ marginTop: 12, fontSize: '0.72rem', color: 'rgba(var(--ig-ink-rgb),0.3)', fontStyle: 'italic' }}>
  ...
  return (
    <div style={S.listPanel}>
      <div style={S.listHeader}>
        <div style={{ fontWeight: 700, fontSize: '0.9rem', color: 'var(--ig-txt)' }}>Recent Analyses</div>
        <div style={{ fontSize: '0.72rem', color: 'rgba(var(--ig-ink-rgb),0.35)' }}>{items.length} messages</div>
      </div>
          ...
          return (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                <span style={{ fontSize: '0.72rem', color: 'rgba(var(--ig-ink-rgb),0.45)' }}>
                  {m.display_name}
                </span>
                {m.dominant !== '—' && (
                  <span style={{
              ...
              <div style={{
                fontSize: '0.8rem', color: 'rgba(var(--ig-ink-rgb),0.75)',
                overflow: 'hidden', textOverflow: 'ellipsis',
                display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical',
              }}>
                {m.text || <span style={{ color: 'rgba(var(--ig-ink-rgb),0.3)', fontStyle: 'italic' }}>No text</span>}
              </div>
              {!m.has_pipeline && (
                <div style={{ fontSize: '0.64rem', color: 'rgba(var(--ig-ink-rgb),0.25)', marginTop: 4 }}>
                  ⚠ No pipeline data
                </div>
              )}
            </div>
...
export default function AdminPipelinePage({ currentUser, onBack }) {
  ...
  return (
    <div style={S.page} data-ig-theme={igTheme}>
      <div style={S.topBar}>
          ...
          {detail && !loadingDetail && (
            <>
              <div style={S.msgHeader}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '0.68rem', color: 'rgba(var(--ig-ink-rgb),0.35)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.07em' }}>
                    {detail.message.display_name} · {fmt(detail.message.timestamp)} · conv {detail.message.conversation_id}
                  </div>
                  <div style={{ fontSize: '1.05rem', color: 'var(--ig-txt)', fontWeight: 500, lineHeight: 1.5 }}>
                    "{detail.message.text}"
                  </div>
                </div>
                {detail.ground_truth && (
...
const S = {
  page: {
    ...
    background: 'var(--ig-bg)', color: 'var(--ig-txt)', fontFamily: 'inherit', overflow: 'hidden',
  },
  topBar: {
    display: 'flex', alignItems: 'center', gap: 16,
    padding: '14px 24px', borderBottom: '1px solid rgba(var(--ig-ink-rgb),0.08)',
    background: 'rgba(var(--ig-ink-rgb),0.02)', flexShrink: 0,
  },
  backBtn: {
    padding: '7px 14px', background: 'rgba(var(--ig-ink-rgb),0.07)',
    border: '1px solid rgba(var(--ig-ink-rgb),0.12)', borderRadius: 8,
    color: 'var(--ig-txt)', cursor: 'pointer', fontSize: '0.82rem',
  },
  pageTitle: { fontSize: '1rem', fontWeight: 700, color: 'var(--ig-txt)', marginRight: 10 },
  pageSub:   { fontSize: '0.75rem', color: 'rgba(var(--ig-ink-rgb),0.4)' },
  refreshBtn: {
  ...
  listPanel: {
    width: 280, flexShrink: 0, borderRight: '1px solid rgba(var(--ig-ink-rgb),0.07)',
    display: 'flex', flexDirection: 'column', overflow: 'hidden',
    background: 'rgba(var(--ig-ink-rgb),0.01)',
  },
  listHeader: {
    padding: '14px 14px 8px', display: 'flex', justifyContent: 'space-between',
    alignItems: 'center', borderBottom: '1px solid rgba(var(--ig-ink-rgb),0.06)',
  },
  searchInput: {
    width: '100%', padding: '8px 10px', boxSizing: 'border-box',
    background: 'rgba(var(--ig-ink-rgb),0.06)', border: '1px solid rgba(var(--ig-ink-rgb),0.1)',
    borderRadius: 8, color: 'var(--ig-txt)', fontSize: '0.8rem', outline: 'none',
  },
  listItem: {
    padding: '10px 14px', cursor: 'pointer', borderBottom: '1px solid rgba(var(--ig-ink-rgb),0.04)',
  ...
  placeholder: {
    display: 'flex', alignItems: 'center', justifyContent: 'center',
    height: '60%', color: 'rgba(var(--ig-ink-rgb),0.3)', fontSize: '0.9rem',
  },
  msgHeader: {
    display: 'flex', gap: 16, alignItems: 'flex-start',
    padding: '16px 20px', background: 'rgba(var(--ig-ink-rgb),0.03)',
    border: '1px solid rgba(var(--ig-ink-rgb),0.08)', borderRadius: 14,
    marginBottom: 24,
  },
  ...
  stepLine: {
    width: 1, flex: 1, background: 'rgba(var(--ig-ink-rgb),0.07)', minHeight: 16, marginTop: 4,
  },
  stepLabel: {
    fontSize: '0.68rem', fontWeight: 600, color: 'rgba(var(--ig-ink-rgb),0.4)',
    textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 8,
  },
  stageCard: {
    background: 'rgba(var(--ig-ink-rgb),0.03)', border: '1px solid rgba(var(--ig-ink-rgb),0.08)',
  ...
  stageDesc:  { fontSize: '0.68rem', color: 'rgba(var(--ig-ink-rgb),0.35)', marginTop: 2 },
  emptyMsg:   { fontSize: '0.78rem', color: 'rgba(var(--ig-ink-rgb),0.3)', textAlign: 'center', padding: '16px 0' },
  miniLabel:  { fontSize: '0.65rem', color: 'rgba(var(--ig-ink-rgb),0.35)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 8 },
  contextRow: { display: 'flex', alignItems: 'center' },
  ctxHint:    { fontSize: '0.70rem', color: 'rgba(var(--ig-ink-rgb),0.45)', lineHeight: 1.5, margin: '0 0 8px' },
  ctxLabel:   { fontSize: '0.75rem', color: 'rgba(var(--ig-ink-rgb),0.5)', whiteSpace: 'nowrap' },
  ...
  contextChip: {
    ...
    fontSize: '0.75rem', color: 'rgba(var(--ig-ink-rgb),0.75)',
  },
};
