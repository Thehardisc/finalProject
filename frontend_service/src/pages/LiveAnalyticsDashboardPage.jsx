...
function KpiCard({ label, value, sub, color = '0,119,255' }) {
  return (
    <div style={{
      ...
      borderRadius: 16,
      boxShadow: `inset 0 1px 0 rgba(255,255,255,0.9), 0 2px 12px rgba(${color},0.06)`,
    }}>
      <div style={{ fontSize: '0.72rem', fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 6 }}>
        {label}
      </div>
      <div style={{ fontSize: '2rem', fontWeight: 800, color: `rgb(${color})`, lineHeight: 1, marginBottom: 4 }}>
        {value}
      </div>
      {sub && <div style={{ fontSize: '0.74rem', color: '#9ca3af' }}>{sub}</div>}
    </div>
  );
}
...
function EmotionBar({ emotion, count, total }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
      <div style={{ width: 90, fontSize: '0.78rem', color: '#374151', textTransform: 'capitalize', textAlign: 'right', flexShrink: 0 }}>
        {emotion}
      </div>
      <div style={{ flex: 1, height: 10, borderRadius: 99, background: 'rgba(0,0,0,0.06)', overflow: 'hidden' }}>
        <div style={{
      ...
      <div style={{ width: 46, fontSize: '0.72rem', color: '#9ca3af', textAlign: 'right', flexShrink: 0 }}>
        {count} <span style={{ color: '#d1d5db' }}>({pct.toFixed(0)}%)</span>
      </div>
    </div>
  );
}
...
export default function LiveAnalyticsDashboardPage({ currentUser, onBack }) {
  ...
  const timelineOpts = {
    ...
    scales: {
      y: { min: 0, max: 100, ticks: { color: '#9ca3af', font: { size: 10 } }, grid: { color: 'rgba(0,0,0,0.04)' } },
      x: { ticks: { color: '#9ca3af', font: { size: 9 }, maxTicksLimit: 10 }, grid: { display: false } },
    },
  ...
  return (
    <div style={{
      minHeight: '100vh',
      background: 'linear-gradient(135deg,#f0f4ff 0%,#faf8ff 60%,#fff0f8 100%)',
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={onBack} style={{
            ...
            width: 36, height: 36, cursor: 'pointer', fontSize: '1rem', color: '#374151',
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {filterEmo && (
            <button onClick={() => setFilterEmo(null)} style={{
              padding: '6px 12px', borderRadius: 8, border: '1px solid rgba(0,0,0,0.10)',
        ...
        <main style={{ padding: '24px 32px 48px', display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={layer}>
              ...
              <div style={{ height: 210 }}>
                {timeline.length > 1
                  ...
                  : <p style={{ color: '#9ca3af', margin: 0, fontSize: '0.85rem' }}>Need more messages to draw timeline.</p>}
              </div>
            </div>
            ...
            <div style={layer}>
              ...
              <div style={{ height: 210 }}>
                {top8.length > 0
                  ...
                  : <p style={{ color: '#9ca3af', margin: 0 }}>No data yet.</p>}
              </div>
            </div>
          </div>
          ...
          <div style={layer}>
            <h4 style={{ margin: '0 0 16px', fontSize: '0.78rem', fontWeight: 700, color: '#374151', textTransform: 'uppercase', letterSpacing: '0.07em' }}>
              ...
              <span style={{ float: 'right', fontWeight: 400, color: '#9ca3af', textTransform: 'none', letterSpacing: 0 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexShrink: 0 }}>
                      ...
                      <div style={{ fontSize: '0.70rem', color: '#9ca3af' }}>
                        {item.display_name}
                      </div>
                      <div style={{ fontSize: '0.70rem', color: '#d1d5db' }}>
              ...
              {feed.length === 0 && (
                <p style={{ color: '#9ca3af', margin: 0, textAlign: 'center', padding: '32px 0', fontSize: '0.85rem' }}>
